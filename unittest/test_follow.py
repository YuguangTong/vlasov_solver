...
from py_vlasov.follow_parameter import (
    follow_kz, follow_kp, follow_k, follow_kzkp, follow_angle, follow_beta,
...
class Test_follow(unittest.TestCase):
    ...
    def test_follow_kzkp_alfven_1(self):
        """
        test follow_kzkp() on an alven mode.
        ...
        target_kzkp = [0.1*np.sqrt(3.), 0.1]
        freq = follow_kzkp(seed_freq, target_kzkp, param)
        freq = freq[0]        
        w_r, w_i = real_imag(freq)
        npt.assert_allclose(w_r, 0.1639, rtol = 1e-3)
        npt.assert_allclose(w_i, -2.967e-3, rtol = 1e-3)

        target_kzkp = [0.5, 0.5]
        freq = follow_kzkp(seed_freq, target_kzkp, param)
        freq = freq[0]        
        w_r, w_i = real_imag(freq)
        npt.assert_allclose(w_r, 0.348035, rtol = 1e-3)
        npt.assert_allclose(w_i, -0.126309, rtol = 1e-3)
